...
def test_default_versions_do_not_auto_claim_newer_2_1_releases():
    # DEFAULT_VERSION_RANGES must not claim a 2.1.x release until Docker patch
    # smoke has proven it boots. Bump this literal alongside the ceiling in
    # _pinned_default.py after a green smoke run on the next release.
    assert not any(version_in_range("2.1.216", tested) for tested in DEFAULT_VERSION_RANGES)
